describe Etl::Actions::Json::ClientApplicationsAction do
  class ClientApplication; end
  ...
  context 'succeeds' do
    ...
    it 'sets the guids' do
      expect(ctx.unit_groups_hash.first['discount_plans'].first['client_applications'].first.keys).to include('guid')
    end
  end
  ...
  context 'next' do
    let(:unit_groups_hash) do
      ...
      unit_groups_hash.first['discount_plans'].first['client_applications'] = []
      unit_groups_hash
    end
    ...
    it 'do nothing when nil' do
      expect(ctx.unit_groups_hash.first['discount_plans'].first['client_applications'].empty?).to be_truthy
    end
  end
end
