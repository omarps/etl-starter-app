describe Etl::Actions::Json::DiscountsAction do
  class Discount; end
  ...
  context 'succeeds' do
    ...
    it 'sets the guids' do
      expect(ctx.unit_groups_hash.first['discount_plans'].first['discount_plan_discounts'].first.keys).to include('guid')
    end
  end
  ...
  context 'next' do
    let(:unit_groups_hash) do
      ...
      unit_groups_hash.first['discount_plans'].first['discount_plan_discounts'] = []
      unit_groups_hash
    end
    ...
    it 'do nothing when nil' do
      expect(ctx.unit_groups_hash.first['discount_plans'].first['discount_plan_discounts'].empty?).to be_truthy
    end
  end
end
